...
- Обработчики событий формы (`ПриОткрытии`, `ПередЗакрытием` и т.д.) нельзя объявлять как `Асинч` — параметр `Отказ` будет проигнорирован
- Для асинхронных операций при открытии/закрытии — вызывать отдельную `Асинч`-процедуру из обычного обработчика
- Обработчики команд (`&НаКлиенте`) — можно безопасно делать `Асинч`
