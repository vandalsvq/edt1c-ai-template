...
# /skd-edit — точечное редактирование СКД (Template.dcs)
...
| Параметр | Описание |
|----------|----------|
| `TemplatePath` | Путь к Template.dcs (или к папке — автодополнение Ext/Template.dcs) |
...
```bash
python3 .claude/skills/skd-edit/scripts/skd-edit.py -TemplatePath "<path>" -Operation <op> -Value "<value>"
```
...
Не поддерживает пакетный режим. Value — полный текст запроса или `@path/to/file.sql` (ссылка на внешний файл). Путь разрешается относительно Template.dcs, затем CWD.
