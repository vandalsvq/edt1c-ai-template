...
argument-hint: "[-DefinitionFile <json> | -Value <json-string>] -OutputPath <Template.dcs>"
allowed-tools:
  - Bash
  - Read
  - Write
...
Принимает JSON-определение схемы компоновки данных → генерирует Template.dcs (DataCompositionSchema).
...
| `OutputPath` | Путь к выходному Template.dcs |

```bash
# Из файла
python3 .claude/skills/skd-compile/scripts/skd-compile.py -DefinitionFile "<json>" -OutputPath "<Template.dcs>"

# Из строки (без промежуточного файла)
python3 .claude/skills/skd-compile/scripts/skd-compile.py -Value '<json-string>' -OutputPath "<Template.dcs>"
```
