...
Читает Template.mxlx табличного документа и выводит компактную сводку: именованные области, параметры, наборы колонок. Заменяет необходимость читать тысячи строк XML.
...
|---------------|:------------:|--------------|------------------------------------------|
| TemplatePath  | нет          | —            | Прямой путь к Template.mxlx               |
...
```bash
python3 .claude/skills/mxl-info/scripts/mxl-info.py -TemplatePath "<путь>"
```

Или по имени обработки/макета:
```bash
python3 .claude/skills/mxl-info/scripts/mxl-info.py -ProcessorName "<Имя>" -TemplateName "<Макет>" [-SrcDir "<каталог>"]
```
